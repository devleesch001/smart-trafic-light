...
extern I2C_HandleTypeDef I2cHandle; // serial monitor
...
int _I2CWrite(VL53L0X_DEV Dev, uint8_t *pdata, uint32_t count) {
    ...
    status = HAL_I2C_Master_Transmit(&hi2c1, Dev->I2cDevAddr, pdata, count, i2c_time_out);
    if (status) {
...
int _I2CRead(VL53L0X_DEV Dev, uint8_t *pdata, uint32_t count) {
    ...
    status = HAL_I2C_Master_Receive(&hi2c1, Dev->I2cDevAddr|1, pdata, count, i2c_time_out);
    if (status) {
        //VL6180x_ErrLog("I2C error 0x%x %d len", dev->I2cAddr, len);
        //XNUCLEO6180XA1_I2C1_Init(&hi2c1);
    }
